#!/usr/bin/env python3
"""
Corrected test script with proper state tracking
"""
...
def simulate_eth_price_movement_corrected():
    """Simulate ETH price movement with corrected state tracking"""
    ...
    last_alert = None
    alert_count = 0
    previous_state = "above"  # Track the actual previous state
    ...
    for i, (price_desc, price, expected) in enumerate(price_movements, 1):
        ...
        if should_trigger:
            alert_count += 1
            # Create alert with the actual previous state
            last_alert = Alert(
                ...
                previous_state=previous_state,  # Use the actual previous state
