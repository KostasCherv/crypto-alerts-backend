#!/usr/bin/env python3
"""
Corrected test script to verify the complete system works with the fix
"""
...
def simulate_eth_price_movement_corrected():
    """Simulate ETH price movement and test continuous alerts with correct logic"""
    print("🚀 Testing Corrected ETH Continuous Alert System")
    print("=" * 60)
    ...
    last_alert = None
    alert_count = 0
    current_state = "above"  # Track current state: "above" or "below"
    ...
    for i, (price_desc, price, expected) in enumerate(price_movements, 1):
        ...
        new_state = "above" if price >= eth_alert.target_price else "below"
        
        should_trigger = should_trigger_alert_fixed(eth_alert, price, last_alert)
        
        status = "🚨 ALERT!" if should_trigger else "ℹ️  No alert"
        state_change = f"({current_state} → {new_state})" if current_state != new_state else f"({new_state})"
...
if __name__ == "__main__":
    simulate_eth_price_movement_corrected()